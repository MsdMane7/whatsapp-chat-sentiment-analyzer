...
def fetch_stats_analysis(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
...
def most_busy_users_analysis(df):
...
def create_wordcloud_analysis(selected_user, df):
    f = open('stop_hinglish.txt', 'r')
    stop_words = f.read()
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
    ...
    def remove_stop_words_analysis(message):
        y = []
        for word in message.lower().split():
            if word not in stop_words:
                y.append(word)
    ...
    temp['message'] = temp['message'].apply(remove_stop_words_analysis)
    df_wc = wc.generate(temp['message'].str.cat(sep=" "))
    return df_wc


def most_common_words_analysis(selected_user, df):
    f = open('stop_hinglish.txt', 'r')
    stop_words = f.read()
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
...
def emoji_helper_analysis(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
